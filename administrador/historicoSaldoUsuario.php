...
<body>
	...
	<section>
		...
		<?php
		$idUsuarioSel = @$_GET['idUsuario'];
		if (isset($idUsuarioSel)) {
			if (strlen($idUsuarioSel)==0) {
				$_SESSION['idUsuarioSel'] = NULL;
				$idUsuarioSel = NULL;
			} else {
				$_SESSION['idUsuarioSel'] = $idUsuarioSel;
			}
		} else {
			$idUsuarioSel = @$_SESSION['idUsuarioSel'];
		}
		...
		?>
					...
					<div id="listadoProductos">
						...
						?>
						<span id="saldo2">
							SALDO CONFIRMADO:<br/> <b><?=$saldo?> &euro;</b>
						</span>
						<span id="saldo3">
							SALDO TRAS PEDIDOS NO FINALIZADOS:<br/><b> <?=$saldoPendiente?> &euro;</b>
						</span>
			...
			?>
					...
					if(@$numrows != 0)
					{
						...
						?><div class="grid-12 grid paginationCV" style="position:relative; margin-top: 10px; margin-bottom: 10px"><ul><?php
					         ...
					         <?php
					              ...
					              {?>
					                <li><a href="historicoSaldoUsuario.php?page=<?php echo $i;?>"><?php echo $i;?></a></li>
					        <?php }
					           
					           //Y SI LA ULTIMA PÁGINA ES MAYOR QUE LA ACTUAL MUESTRO EL BOTON NEXT O LO DESHABILITO
					            if($lastpage >$page )
					            {?>      
					                <li class="next"><a href="historicoSaldoUsuario.php?page=<?php echo $nextpage;?>" >&rarr;</a></li><?php
					            }
					            else
					            {?>
					        ...
					        ?>
					            <li class="previous"><a href="historicoSaldoUsuario.php?page=<?php echo $prevpage;?>">&larr;</a></li><?php
					            ...
					            ?>       <li class="active"><?php echo $i;?></li><?php
					                ...
					                {
					            ?>       <li><a href="historicoSaldoUsuario.php?page=<?php echo $i;?>" ><?php echo $i;?></a></li><?php
					                }
					            }
				...
				<br/>
		<div id="botonera">
			<input id="actualiza" name="actualiza"  type="button" value="Comprobar Saldo" onclick="$.blockUI({ message: 'Por favor espere...' });document.location='actualizaSaldoUsuario.php?idUsuario=<?=$idUsuarioSel?>'" />
			<input id="cancel" name="cancel"  type="button" value="Volver" onclick="document.location='usuarios.php?page=<?=$pageUserSel?>'" />
		</div>
	</section>
</body>
</html>